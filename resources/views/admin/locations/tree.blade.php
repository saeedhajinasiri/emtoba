...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div id="nestable-location" class="m-b-20">
                <button type="button" data-action="expand-all" class="btn btn-success"> {{ trans('admin.locations.expandAll') }} </button>
                <button type="button" data-action="collapse-all" class="btn btn-success"> {{ trans('admin.locations.collapseAll') }} </button>
            </div>
        </div>
            ...
            <div class="panel panel-bd lobidisable">
                <div class="panel-heading">
                    <div class="panel-title">
                        <h4>{{ trans('admin.locations.list') }}</h4>
                    </div>
                </div>
                <div class="panel-body">
                    <form action="{{ route('admin.locations.sort') }}" method="post">
                        {{ csrf_field() }}
                        <div class="portlet-body">
                            <div class="dd-content" data-id="{{ $root['id'] }}">
                                <span>{{ $root['title_fa'] }}</span>
                                <a class="btn btn-xs btn-icon-only pull-left quickCreate tooltips" data-placement="top" data-original-title="{{ trans('admin.locations.quickCreate') }}" href="javascript:;">
                                    <i class="ti-plus"></i>
                                </a>
                            </div>

                            <div class="dd" id="locationList">
                                <ol class="dd-list">
        ...
        <div class="col-sm-12 col-md-6">
            <div class="panel panel-bd lobidisable">
                <div class="panel-heading">
                    <div class="panel-title">
                        <h4>{{ trans('admin.locations.form') }}</h4>
                    </div>
                </div>
        ...
        $(document).ready(function () {
            ...
            $this.nestable({
                dropCallback: function (details) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '{{ route('admin.locations.sort') }}',
                        dataType: 'json',
                        data: {
                            id: details.sourceId,
            ...
            $body.on("click", '.quickEdit', function (e) {
                ...
                $.get(url, function (data, item) {
                    ...
                    $('#quickForm').on('submit', function (e) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '/admin/locations/' + id + '/quickUpdate',
                            data: $(this).serialize(),
            ...
            $body.on("click", '.quickCreate', function (e) {
                ...
                $.get('{{ route('admin.locations.quickCreate') }}', function (data, item) {
                    $('#quickEdit').html(data);
                    $('#parent_id').val(id);
            ...
            $body.on("click", '.quickDestroy', function () {
                if (confirm('{!! trans('admin.locations.areYouSure') !!}') == true) {
                    //blockPage();
            ...
            $('#nestable-location').on('click', function (e) {
                var target = $(e.target),
